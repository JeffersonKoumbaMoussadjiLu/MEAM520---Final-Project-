...
class StaticGrabber():
    def __init__(self, detector, arm, team, ik, fk):
        ...
        if team == 'blue':
            ...
            self.H_ee_camera[0,-1] += 0.025 #0.025
            self.H_ee_camera[1,-1] += 0.00 #0.0
...
class DynamicGrabber():
    """Class for grabbing blocks from the rotating turntable"""
    def __init__(self, detector, arm, team, ik, fk):
        self.detector = detector
        self.arm = arm
        self.team = team
        ...
        if team == 'blue':
            ...
            # Define positions for blue team
            self.pre_pose = np.array([0.56, 0.0, 0.0, -1.8, 0.0, 1.3, 0.75])
            self.wait_pose = np.array([0.8, 0.0, 0.0, -1.3, 0.0, 0.9, 0.75])
            self.initial_pose = np.array([0.0, 0.3, 0.0, -1.8, 0.0, 1.4, 0.75])
            self.put_pose = np.array([0.0, 0.3, 0.0, -1.6, 0.0, 1.6, 0.75])
            self.over_pose = np.array([0.0, 0.4, 0.0, -1.8, 0.0, 1.8, 0.75])
            
        else:  # red team
            self.H_ee_camera[0,-1] += 0.035
            self.H_ee_camera[1,-1] += 0.005
            self.H_ee_camera[2,-1] += 0.005
            
            # Define positions for red team
            self.pre_pose = np.array([0.56, 0.0, 0.0, -1.8, 0.0, 1.3, 0.25])
            self.wait_pose = np.array([0.8, 0.0, 0.0, -1.3, 0.0, 0.9, 0.25])
            self.initial_pose = np.array([0.0, -0.3, 0.0, -1.8, 0.0, 1.4, 0.25])
            self.put_pose = np.array([0.0, -0.3, 0.0, -1.6, 0.0, 1.6, 0.25])
            self.over_pose = np.array([0.0, -0.4, 0.0, -1.8, 0.0, 1.8, 0.25])
        
    def move_to_pre_pose(self):
        """Move to position to observe the turntable"""
        print("Moving to pre-observation position...")
        self.arm.safe_move_to_position(self.pre_pose)
        
    def move_to_wait_pose(self):
        """Move to position where we'll wait to grab a block"""
        print("Moving to wait position...")
        self.arm.safe_move_to_position(self.wait_pose)
        
    def detect_dynamic_blocks(self):
        """Detect dynamic blocks on the turntable"""
        result_list = []
        q = self.arm.get_positions()
        _, current_H = self.fk.forward(q)
        
        for (name, pose) in self.detector.get_detections():
            # Only consider dynamic blocks (yellow)
            if "dynamic" in name.lower() or "yellow" in name.lower():
                # Transform from camera frame to world frame
                world_pose = np.matmul(current_H, np.matmul(self.H_ee_camera, pose))
                result_list.append(world_pose)
                
        print(f"Detected {len(result_list)} dynamic blocks")
        return result_list
    
    def wait_unitl_grabbed(self):
        """Wait for a block to come into position and grab it"""
        print("Waiting for dynamic block...")
        
        # Open gripper wide
        self.arm.open_gripper()
        
        # Wait for a short time to allow block to approach
        sleep(2.0)
        
        # Close gripper quickly to catch block
        print("Attempting to grab dynamic block...")
        self.arm.exec_gripper_cmd(0.04, 100)  # More force for dynamic grab
        
        # Check if we grabbed anything
        gripper_state = self.arm.get_gripper_state()
        print(f"Gripper position after grab: {gripper_state['position']}")
        
        # If gripper is too closed, we didn't catch anything
        if gripper_state['position'] < 0.03:
    ...
    def move_to_initial_pose(self):
        """Move to a safe position with the grabbed block"""
        print("Moving to initial position...")
        self.arm.safe_move_to_position(self.initial_pose)
        
    def put(self):
        """Place the dynamic block on static platform"""
        print("Placing dynamic block on static platform...")
        
        # Move to position over the static platform
        self.arm.safe_move_to_position(self.put_pose)
        
        # Release the block
        self.arm.open_gripper()
        
        # Move up slightly to avoid collisions
        safe_pose = deepcopy(self.put_pose)
        safe_pose[3] += 0.3  # Adjust joint 4
        self.arm.safe_move_to_position(safe_pose)
        
    def get_over(self):
        """Move to position to view the static platform"""
        print("Moving to position to view static platform...")
        self.arm.safe_move_to_position(self.over_pose)
...
if __name__ == "__main__":
    ...
    # First grab static blocks
    print("Grabbing static blocks!!\n")
    static_blocks_grabbed = 0
    ...
    for i in range(4):
        ...
        static_blocks_grabbed += 1
        print(f"Successfully grabbed static block {static_blocks_grabbed}!\n")
    
    # Ensure gripper is open after static block grabbing
    arm.open_gripper()
    
    # Now grab dynamic blocks
    print("\nGrabbing dynamic blocks!!\n")
    dynamic_blocks_grabbed = 0
    
    for i in range(4):
        # 1. Move to position to observe turntable
        dynamic_grabber.move_to_pre_pose()
        sleep(1)
        
        # 2. Move to wait position
        dynamic_grabber.move_to_wait_pose()
        
        # 3. Wait and grab block
        success = dynamic_grabber.wait_unitl_grabbed()
        if not success:
            print("Failed to grab dynamic block, trying again")
            i -= 1
            continue
        
        # 4. Move to a safe position with block
        dynamic_grabber.move_to_initial_pose()
        
        # 5. Place block on static platform
        dynamic_grabber.put()
        
        # 6. Move to position to see static platform
        dynamic_grabber.get_over()
        
        # 7. Now use static grabber to pick up block and stack it
        sleep(1)  # Give time for robot to stabilize
        
        # Try to detect the block we just placed
        result_list = static_grabber.detect_and_convert()
        if len(result_list) == 0:
            print("Could not detect placed dynamic block")
            continue
            
        # Find the closest block (should be the one we just placed)
        target_H = static_grabber.find_closest(result_list)
        
        # Grab it
        static_grabber.grab(target_H, 0)  # Use position 0 as reference
        
        # Place on the stack
        stack_position = static_blocks_grabbed + dynamic_blocks_grabbed
        static_grabber.put(stack_position)
        
        dynamic_blocks_grabbed += 1
        print(f"Successfully grabbed and stacked dynamic block {dynamic_blocks_grabbed}!\n")
    ...
    # Print summary
    print(f"\nTask completed!")
    print(f"Static blocks grabbed: {static_blocks_grabbed}")
    print(f"Dynamic blocks grabbed: {dynamic_blocks_grabbed}")
    print(f"Total blocks stacked: {static_blocks_grabbed + dynamic_blocks_grabbed}")
