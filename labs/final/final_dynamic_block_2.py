...
class DynamicGrabber():
    """
    Single direct vantage‐and‐grab approach, no partial stepping.
    """
    def __init__(self, detector, arm, team, ik, fk):
        self.detector = detector
        self.arm      = arm
        ...
        if self.team == 'blue':
            ...
            #                          -0.2694, 1.6976, -0.8327 ]) #sideways
            self.pre_pose =  np.array([ 0.84, -0.88, -1.8, -1.15, -0.84, 1.47, -0.42])
            ...
            self.pre_pose = np.array([ -0.83, -0.93, 1.72, -1.14, 0.93, 1.43, -1.18])

            #self.pre_pose = np.array([ 0.9728-0.3,  1.2579,  0.75,  -0.8674,
            #                            0.5607, 1.6436, -1.1162 ]) #sideways
        ...
        # A place pose after grabbing
        self.place_pose = np.array([0.5, -1.0, 0.0, -1.8, 0.0, 1.5, 0.5])

        # Camera->EE transform from your calibration
        self.H_ee_camera = detector.get_H_ee_camera()

        """
        # Slight offsets if needed
        if team == 'blue':
            self.H_ee_camera[0,-1] -= 0.035
            self.H_ee_camera[1,-1] += 0.005
            self.H_ee_camera[2,-1] += 0.015
    ...
    def put(self):
        self.arm.safe_move_to_position(self.place_pose)
        self.arm.open_gripper()

    # The direct vantage‐and‐grab approach:
    def vantage_and_grab(self):
        """
        Move to vantage pose, detect the block once, do a single direct approach.
        """
        # 1) vantage
        self.arm.safe_move_to_position(self.pre_pose)
        rospy.sleep(1.0)  # let camera settle

        # 2) get current arm transform
        q_now = self.arm.get_positions()
        _, H_base_ee = self.fk.forward(q_now)

        # 3) detect blocks
        detections = self.detector.get_detections()
        if not detections:
            print("No dynamic blocks seen from vantage—cannot grab.")
            return False

        # 4) convert to base frame
        T_blocks = []
        for (_, pose_cam) in detections:
            T_blocks.append(H_base_ee @ self.H_ee_camera @ pose_cam)

        block_T = self.find_closest(T_blocks)
        block_pos = block_T[:3,3]
        print(f"Closest block at: {block_pos}")

        # 5) build final approach transform—EE pointing down at block
        T_final = np.eye(4)
        T_final[:3,:3] = self.downward_orientation()
        T_final[0,3]   = block_pos[0]
        T_final[1,3]   = block_pos[1]
        T_final[2,3]   = block_pos[2]

        # 6) IK
        q_final = self.solve_ik(T_final, q_now)
        if q_final is None:
            print("Direct approach IK failed—out of reach/orient.")
            return False

        # 7) Move + close
        self.arm.safe_move_to_position(q_final)
        self.arm.exec_gripper_cmd(0.03, 80)
        rospy.sleep(0.5)  # wait or check sensors

        # 8) Lift
        q_lift = deepcopy(q_final)
        q_lift[1] -= 0.3
        self.arm.safe_move_to_position(q_lift)
        print("Direct vantage‐and‐grab success!")
        return True
    ...
    def solve_ik(self, T_des, q_seed):
        q_sol, _, success, _ = self.ik.inverse(
            T_des, q_seed, method='J_pseudo', alpha=0.75)
...
if __name__ == "__main__":
    ...
    #print("\nNow let's try a direct vantage & grab for the dynamic block!\n")
    success = dynamic_grabber.vantage_and_grab()
    if success:
        dynamic_grabber.put()
        print("Dynamic block grabbed & placed!")
    else:
        print("Direct vantage approach failed to grab block.")
