...
class JacobianDemo():
    ...
    def eight(t,fx=0.25,fy=0.5,rx=.15,ry=.1):
        """
    ...
    def ellipse(t,f=0.25,ry=.15,rz=.10):
        ...
        y_t = y_c + ry*cos(f*t)
        z_t = z_c + rz*sin(f*t)
        xdes = np.array([x_c, y_t, z_t])

        # Ellipse velocity (derivative wrt time)
        vy_t = -ry*f*sin(f*t)
        vz_t =  rz*f*cos(f*t)
        vdes = np.array([0.0, vy_t, vz_t])

        # Example orientation trajectory (rotate about Z)
        # angle(t) = 0.3 * sin(f*t)
        angle = 0.3 * sin(f*t)
        rotvec = angle * np.array([0, 0, 1])       # axis = z-hat
        Rdes = rotvec_to_matrix(rotvec)

        # Angular velocity = derivative of angle(t) * axis
        dangle_dt = 0.3 * f * cos(f*t)
        ang_vdes = dangle_dt * np.array([0, 0, 1])
    ...
    def line(t,f=0.25,L=.15):
        ...
        # Sinusoid along z
        z_t = x0[2] + (L/2.0)*sin(f*t)
        vz_t = (L/2.0)*f*cos(f*t)
        xdes = np.array([x0[0], x0[1], z_t])
        vdes = np.array([0.0, 0.0, vz_t])

        # Orientation: rotating about x-axis (example from stub)
        ang = -np.pi + (np.pi/4.0)*sin(f*t)
        r = ang * np.array([1.0, 0.0, 0.0])
        Rdes = rotvec_to_matrix(r)
