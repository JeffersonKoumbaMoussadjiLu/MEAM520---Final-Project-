...
visulaize_mani_ellipsoid = True #False
...
targets = [
    #transform( np.array([0.2, 0.0, 0.1]), np.array([0, pi, pi]) ),
    transform( np.array([0.8, 0.0, 0.1]), np.array([0, pi, pi]) ),
    #transform( np.array([0.0, 0.0, 0.1]), np.array([0, pi, pi]) ),
]


'''
targets = [
    #transform( np.array([0.8, 0.0, 0.2]), np.array([0, pi, pi]) ),
...
if __name__ == "__main__":
    ...
    for i, target in enumerate(targets):
        ...
        #J_trans #J_pseudo #try both methods
        stop = perf_counter()
        dt = stop - start
